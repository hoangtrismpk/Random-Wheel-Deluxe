...
import type { WheelDynamicBackground, LinearGradientBackgroundConfig, RadialGradientBackgroundConfigForWheel, GradientColorStop } from '../types';
...
const MAX_GRADIENT_STOPS = 5; // Max stops for wheel gradients
const DEFAULT_SOLID_COLOR_WHEEL = '#FFC107'; // A default wheel color (yellowish)
const DEFAULT_LINEAR_GRADIENT_WHEEL: LinearGradientBackgroundConfig = {
  type: 'linear-gradient',
  angle: 45,
  stops: [
    { id: 'lg_wheel_default_1', color: '#FFC107', position: 0 },
    { id: 'lg_wheel_default_2', color: '#FF5722', position: 100 },
  ],
};
const DEFAULT_RADIAL_GRADIENT_WHEEL: RadialGradientBackgroundConfigForWheel = {
  type: 'radial-gradient',
  shape: 'circle',
  position: 'center center',
  stops: [
    { id: 'rg_wheel_default_1', color: '#4CAF50', position: 0 },
    { id: 'rg_wheel_default_2', color: '#8BC34A', position: 100 },
  ],
};
...
const WheelBackgroundColorPicker: React.FC<WheelBackgroundColorPickerProps> = ({
  ...
  const [solidColor, setSolidColor] = useState<string>(DEFAULT_SOLID_COLOR_WHEEL);
  const [solidColorHex, setSolidColorHex] = useState<string>(DEFAULT_SOLID_COLOR_WHEEL);

  const [linearGradientAngle, setLinearGradientAngle] = useState<number>(DEFAULT_LINEAR_GRADIENT_WHEEL.angle);
  const [linearGradientStops, setLinearGradientStops] = useState<GradientColorStop[]>(
    DEFAULT_LINEAR_GRADIENT_WHEEL.stops.map(s => ({...s, id: s.id || `lg_wheel_stop_init_${Date.now()}_${Math.random().toString(36).substring(2,9)}`}))
  );

  const [radialGradientShape, setRadialGradientShape] = useState<RadialGradientBackgroundConfigForWheel['shape']>(DEFAULT_RADIAL_GRADIENT_WHEEL.shape);
  const [radialGradientPosition, setRadialGradientPosition] = useState<string>(DEFAULT_RADIAL_GRADIENT_WHEEL.position);
  const [radialGradientStops, setRadialGradientStops] = useState<GradientColorStop[]>(
     DEFAULT_RADIAL_GRADIENT_WHEEL.stops.map(s => ({...s, id: s.id || `rg_wheel_stop_init_${Date.now()}_${Math.random().toString(36).substring(2,9)}`}))
  );
  ...
  useEffect(() => {
    if (currentBackground === null) {
      // When wheel background is reset (null), picker defaults to solid editor
      setBgType('solid');
      setSolidColor(DEFAULT_SOLID_COLOR_WHEEL);
      setSolidColorHex(DEFAULT_SOLID_COLOR_WHEEL);
      // Reset gradient states to their picker defaults
      setLinearGradientAngle(DEFAULT_LINEAR_GRADIENT_WHEEL.angle);
      setLinearGradientStops(DEFAULT_LINEAR_GRADIENT_WHEEL.stops.map(s => ({...s, id: s.id || `lg_wheel_stop_null_${Date.now()}_${Math.random().toString(36).substring(2,9)}`})));
      setRadialGradientShape(DEFAULT_RADIAL_GRADIENT_WHEEL.shape);
      setRadialGradientPosition(DEFAULT_RADIAL_GRADIENT_WHEEL.position);
      setRadialGradientStops(DEFAULT_RADIAL_GRADIENT_WHEEL.stops.map(s => ({...s, id: s.id || `rg_wheel_stop_null_${Date.now()}_${Math.random().toString(36).substring(2,9)}`})));
      ...
      setLinearGradientStops(currentBackground.stops.map(s => ({...s, id: s.id || `lg_wheel_stop_prop_${Date.now()}_${Math.random().toString(36).substring(2,9)}`})));
      ...
      setRadialGradientStops(currentBackground.stops.map(s => ({...s, id: s.id || `rg_wheel_stop_prop_${Date.now()}_${Math.random().toString(36).substring(2,9)}`})));
    }
  }, [currentBackground]);
  ...
  const handleAddGradientStop = (type: 'linear' | 'radial') => {
    ...
    if (stops.length < MAX_GRADIENT_STOPS) {
      ...
      setStops([
        ...stops,
        { id: `stop_wheel_${type}_${Date.now()}_${stops.length + 1}`, color: '#FFFFFF', position: newStopPosition },
      ]);
    } else {
  ...
  const handleApply = () => {
    if (isSpinning) return;
    let newBackground: WheelDynamicBackground = null;
    if (bgType === 'solid') {
      if (/^#[0-9A-F]{6}$/i.test(solidColorHex) || /^#[0-9A-F]{3}$/i.test(solidColorHex)) {
        newBackground = solidColorHex;
      } else {
        newBackground = solidColor; // fallback to color picker's direct value
      }
  ...
  const handleRemoveCustomBackground = () => {
    if (isSpinning) return;
    onBackgroundChange(null); // This signals to use default wheel appearance (segment colors)
  };
  
  const renderPreview = () => {
    let backgroundStyle = '';
    if (bgType === 'solid') {
      backgroundStyle = (/^#[0-9A-F]{6}$/i.test(solidColorHex) || /^#[0-9A-F]{3}$/i.test(solidColorHex)) ? solidColorHex : solidColor;
    ...
    return (
      <div className="w-full h-20 rounded-md border border-slate-600 shadow-inner" style={{ background: backgroundStyle }}>
        {!backgroundStyle && <div className="flex items-center justify-center h-full text-slate-500 text-xs">Xem trước nền vòng quay</div>}
      </div>
    );
  };
    ...
    return (
      <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-1">
        {stops.map((stop, index) => (
          <div key={stop.id || `wheel-stop-${type}-${index}`} className="flex items-center space-x-2 p-2 bg-slate-700/50 rounded-md">
            <input
              type="color"
              value={stop.color}
            ...
            <input
              ...
              aria-label={`Vị trí của điểm dừng ${index + 1} (phần trăm) cho vòng quay`}
            />
      ...
      {bgType === 'solid' && (
        <div className="space-y-2 p-3 border border-slate-700/50 rounded-md">
          <div className="flex items-center space-x-3">
            <label htmlFor="wheelSolidColorPickerApp" className="text-sm text-slate-300">Màu:</label>
            <input
              type="color"
              id="wheelSolidColorPickerApp"
              value={solidColor}
      ...
      {bgType === 'linear-gradient' && (
        <div className="space-y-3 p-3 border border-slate-700/50 rounded-md">
          <div className="flex flex-col items-start space-y-1">
            <label htmlFor="wheelLinearAngleApp" className="text-sm text-slate-300">Góc (°):</label>
            <input
              type="number" id="wheelLinearAngleApp" min="0" max="360"
              ...
              aria-label="Góc của linear gradient cho vòng quay"
            />
          </div>
      ...
      {bgType === 'radial-gradient' && (
        <div className="space-y-3 p-3 border border-slate-700/50 rounded-md">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="wheelRadialShapeApp" className="block text-sm text-slate-300 mb-1">Hình dạng:</label>
              <select
                id="wheelRadialShapeApp"
                value={radialGradientShape}
                onChange={(e) => !isSpinning && setRadialGradientShape(e.target.value as 'circle' | 'ellipse')}
                disabled={isSpinning}
                className="w-full p-2 text-sm border border-slate-600 rounded-md bg-slate-700 text-slate-200 focus:ring-1 focus:ring-sky-500 disabled:opacity-60"
                aria-label="Hình dạng của radial gradient cho vòng quay"
              >
            ...
            <div>
              <label htmlFor="wheelRadialPositionApp" className="block text-sm text-slate-300 mb-1">Vị trí tâm:</label>
              <select
                id="wheelRadialPositionApp"
                ...
                aria-label="Vị trí tâm của radial gradient cho vòng quay"
              >
