...
const GiftManagement: React.FC<GiftManagementProps> = ({ giftList, setGiftList, isSpinning }) => {
  ...
  const handleAddGift = () => {
    ...
    if (!newGiftTitle.trim() || !newGiftName.trim() || isNaN(quantity) || quantity <= 0) {
      alert('Vui lòng nhập đầy đủ thông tin hợp lệ cho quà tặng (Tiêu đề, Tên quà, Số lượng > 0).');
      return;
    }
  ...
  const handleRemoveGift = (id: string) => {
    setGiftList(prevList => prevList.filter(gift => gift.id !== id));
  };
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>, targetId: string) => {
    ...
    if (!sourceId || sourceId === targetId || !draggedItemId) {
      setDragOverItemId(null); // Clear highlight even if drop is invalid or on self
      return;
    }
